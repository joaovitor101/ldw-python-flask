...
#função middleware para verificar a autenticação do usuario
def init_app(app):
    @app.before_request
    def check_auth():
    ...
    def login():
        if request.method == 'POST':
            ...
            if user and check_password_hash(user.password, password):
                ...
                flash('Falha no login. Verifique o nome de usuario e senha.', 'danger')
                return redirect(url_for('login'))
    ...
    def caduser():
        if request.method == 'POST':
            ...
            if user:
                ...
                db.session.commit()
                flash('Registro realizado com sucesso ! Você ja pode fazer o login.', 'success')
                return redirect(url_for('login'))
